...
snmax=50;  % clipping limit for SNR
...
em=ef<1e-4*max(ef); % mask for zero noise frames
rm=rf<1e-4*max(rf); % mask for zero reference frames
ef(em|rm)=1;
rf(em|rm)=1;
...
if ~nargout || any (m=='p')
    ...
    axh(1)=gca;
    subplot(312);
    plot((1:length(r))/fs,r,'b',(1:length(vf))*(length(r)/length(vf)/fs),vf,'r');
    ylabel('Reference');
    axh(2)=gca;
    subplot(313);
    snv=snf;
