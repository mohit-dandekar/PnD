...
#########################################################################
#
#  Test Wrapper for unit test ADPCM Encoder
#
...
ADPCM_enc_t ADPCM_enc;

aud_data_t data_in[P*BLOCK_SIZE];
adpcm_data_t data_out_ref[P*BLOCK_SIZE];
adpcm_data_t data_out_dut[P*BLOCK_SIZE];
...
adpcm_data_t temp_buffer[BLOCK_SIZE];
...
int main(void)
{
  ...
  for(i=0;i<P;++i)
  {
   ...
   for(j=0;j<blk_len;++j)
    {
      ADPCM_enc.data_in_buffer[j] = data_in[i*blk_len + j];
    }

    /*ADPCM enc process call*/
    ADPCM_enc_process(&ADPCM_enc);
